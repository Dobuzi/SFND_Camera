...
void magnitudeSobel()
{
    ...
    int nrows = imgBlur.rows-delta;
    int ncols = imgBlur.cols-delta;
    
    float gradients[nrows][ncols];
    ...
    cout << "max grad: " << max_grad << endl;
    cout << "min grad: " << min_grad << endl;
    ...
    for (int i = 0; i < nrows; i++)
    {
        for (int j = 0; j < ncols; j++)
        {
            ...
            magnitude.at<unsigned char>(i, j) = char(abs(norm_grad * 255));
        }
    }
